...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded < 0){
        FixTheTill(cashTill, twenties, tens, fives, ones);
    ...
    if (changeNeeded > 0){
        FixTheTill2(cashTill, savedTwenties, savedTens, savedFives, savedOnes);
        FixTheTill(cashTill, twenties, tens, fives, ones);
...
static void FixTheTill(int[] cashTill, int twenties, int tens, int fives, int ones){
    cashTill[3] -= twenties;
    cashTill[2] -= tens;
    cashTill[1] -= fives;
    cashTill[0] -= ones;
...
static void  FixTheTill2(int[] cashTill, int savedTwenties, int savedTens, int savedFives, int savedOnes){
    cashTill[3] += savedTwenties;
    cashTill[2] += savedTens;
    cashTill[1] += savedFives;
    cashTill[0] += savedOnes;
}
